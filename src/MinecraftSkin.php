...
class MinecraftSkin
{
    const MINECRAFT_SKIN_URL = 'http://skins.minecraft.net/MinecraftSkins/';
    ...
    public function get_skin($user)
    {
        $output = null;
        if ($user != 'steve') {
            $ch = curl_init(self::MINECRAFT_SKIN_URL . $user . '.png');
            ...
            $output = $this->getSteveSkin();
        }
        return $output;
    }

    public function getSteveSkin()
    {
    ...
    public function getFace($user = 'steve')
    {
        if (!file_exists(__DIR__ . '/faces/' . $user . '_face.png')) {
            ...
            imagepng($av, __DIR__."/faces/" . $user . '_face.png');

            imagedestroy($im);
            imagedestroy($av);
        }

        return "/faces/$user" . '_face.png';
    }
}
